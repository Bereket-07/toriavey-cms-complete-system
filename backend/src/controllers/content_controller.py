...
def get_wprm_recipes(limit: int = 20, offset: int = 0):
    """
...
def get_wprm_recipe_by_id(recipe_id: int):
    """
...
def search_wprm_recipes(query: str, limit: int = 20):
    """
...
def get_wprm_recipes_not_generated(limit: int = None, offset: int = 0):
    """
    ...
    try:
        ...
        # OPTIMIZATION: Batch fetch all recipes in one query
        recipe_ids = [s.recipe_id for s in statuses]
        recipes_data = wprm_repo.get_recipes_by_ids(recipe_ids)
        
        # Map recipes by ID for easy lookup
        recipes_map = {r['id']: r for r in recipes_data}
        
        # Combine status with recipe data
        recipes = []
        for status_obj in statuses:
            recipe = recipes_map.get(status_obj.recipe_id)
            if recipe:
                recipe['content_status'] = {
                    'status': status_obj.status,
                    'content_generated': status_obj.content_generated,
...
def get_wprm_recipes_generated_not_posted(limit: int = None, offset: int = 0):
    """
...
def get_wprm_recipes_pending(limit: int = None, offset: int = 0):
    """
...
def approve_wprm_content(recipe_id: int):
    """
...
def decline_wprm_content(recipe_id: int, reason: str = None):
    """
    ...
    try:
        ...
        for status_obj in generated_recipes:
            ...
            if status_obj.generated_content:
                ...
                # Check if ANY platform has empty content or parsed=false
                has_valid_content = False
                for platform, platform_content in content.items():
                    if isinstance(platform_content, dict):
                        # Check if parsed is false (means generation failed)
                        if platform_content.get('parsed') == False:
                            should_reset = True
                            logger.info(f"Recipe {status_obj.recipe_id} has unparsed content for {platform}")
                        
                        # Check for actual content (not empty strings)
                        content_value = (
                            platform_content.get('content', '') or 
                            platform_content.get('caption', '') or 
                            platform_content.get('post', '') or 
                            platform_content.get('tweet', '')
                        )
                        
                        # If content is not empty and not just whitespace
                        if content_value and content_value.strip():
                            has_valid_content = True
                
                # If no valid content found across all platforms, reset
                if not has_valid_content or should_reset:
                    status_repo.reset_to_not_generated(status_obj.recipe_id)
                    reset_count += 1
                    logger.info(f"Reset recipe {status_obj.recipe_id} - empty or unparsed content")
            else:
                # No content at all, reset
                status_repo.reset_to_not_generated(status_obj.recipe_id)
                reset_count += 1
                logger.info(f"Reset recipe {status_obj.recipe_id} - no content")
...
def get_wprm_recipe_status(recipe_id: int):
    """
