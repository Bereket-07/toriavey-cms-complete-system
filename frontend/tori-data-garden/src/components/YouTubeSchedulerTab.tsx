...
export function YouTubeSchedulerTab() {
  ...
  const { data: status, isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${CMS_BACKEND_URL}/api/youtube-scheduler/status`,
      );
  ...
  useEffect(() => {
    if (currentStatus.config) {
      if (currentStatus.config.interval_minutes)
        setIntervalMinutes(currentStatus.config.interval_minutes);
      if (currentStatus.config.max_videos)
        setMaxVideos(currentStatus.config.max_videos);
      if (currentStatus.config.platforms) {
        const newPlatforms = { ...platforms };
        Object.keys(newPlatforms).forEach(
          (k) => (newPlatforms[k as keyof typeof platforms] = false),
        );
        currentStatus.config.platforms.forEach((p: string) => {
          if (p in newPlatforms)
            newPlatforms[p as keyof typeof platforms] = true;
        });
  ...
  const startMutation = useMutation({
    mutationFn: async () => {
      ...
      const endpoint = useCron
        ? "/api/youtube-scheduler/start-cron"
        : "/api/youtube-scheduler/start";
      const body = useCron
  ...
  const stopMutation = useMutation({
    mutationFn: async () => {
      await fetch(`${CMS_BACKEND_URL}/api/youtube-scheduler/stop`, {
        method: "POST",
      });
    },
    onSuccess: () => {
  ...
  const runNowMutation = useMutation({
    mutationFn: async () => {
      await fetch(`${CMS_BACKEND_URL}/api/youtube-scheduler/run-now`, {
        method: "POST",
      });
    },
    onSuccess: () => {
      ...
      toast({
        title: "Checking Now",
        description: "Scanning for new videos...",
      });
    },
  });
              ...
              <div>
                ...
                {currentStatus.is_running ? (
                  <div className="flex items-center gap-2 text-red-50">
                    <Badge
                      variant="outline"
                      className="border-red-200/30 text-white bg-red-500/20"
                    >
                      Next Check
                    </Badge>
                  ...
                  <p className="text-muted-foreground">
                    Not checking for new videos. Start to enable.
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 border-none shadow-lg bg-card/80 backdrop-blur-xl ring-1 ring-border">
          <CardHeader>
            ...
            <CardDescription>
              Configure how we check your YouTube channel (@ToriAvey).
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3 bg-muted/40/50 p-4 rounded-xl border border-border">
                ...
                <p className="text-xs text-muted-foreground">
                  How often to scan for new uploads.
                </p>
              </div>
              ...
              <div className="space-y-3 bg-muted/40/50 p-4 rounded-xl border border-border">
                ...
                <p className="text-xs text-muted-foreground">
                  Max new videos to process per run.
                </p>
              </div>
            </div>
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        setPlatforms({
                          ...platforms,
                          [platform as keyof typeof platforms]: !enabled,
                        })
                      }
                      className={`
              ...
              <div className="mt-4 pt-4 border-t border-border">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Useful if you just uploaded a video and want to process it
