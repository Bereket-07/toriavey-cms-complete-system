...
export default function WPRMPending() {
  ...
  const platforms = {
    instagram: { icon: Instagram, color: "text-pink-500", bgColor: "bg-pink-50", hoverColor: "hover:bg-pink-100", name: "Instagram" },
    twitter: { icon: XIcon, color: "text-black", bgColor: "bg-gray-50", hoverColor: "hover:bg-gray-100", name: "X" },
    facebook: { icon: Facebook, color: "text-blue-600", bgColor: "bg-blue-50", hoverColor: "hover:bg-blue-100", name: "Facebook" },
    tiktok: { icon: Video, color: "text-black", bgColor: "bg-zinc-50", hoverColor: "hover:bg-zinc-100", name: "TikTok" },
    pinterest: { icon: Pin, color: "text-red-600", bgColor: "bg-red-50", hoverColor: "hover:bg-red-100", name: "Pinterest" },
  };
  ...
  // Process data
  const rawContent: PendingContent[] = (data?.recipes || []).map((recipe: any) => ({
    ...recipe,
  ...
  const content = rawContent.filter(item => {
    // Search Filter
    const matchesSearch = item.recipe_name.toLowerCase().includes(searchQuery.toLowerCase());

    // Platform Filter
    const matchesPlatform = activePlatformFilter === 'all'
      ? true
      : item.generated_content && Object.keys(item.generated_content).includes(activePlatformFilter);
  ...
  const postContent = async (recipeId: number, platform: string) => {
    ...
    try {
      ...
      const caption = platformContent.caption || platformContent.post || platformContent.tweet || platformContent.description || '';
      const hashtags = platformContent.hashtags || platformContent.keywords || [];
      ...
      if (result.success && result.posted_platforms?.includes(platform)) {
        ...
        queryClient.setQueryData(['wprm-pending'], (oldData: any) => {
          ...
          return {
            ...
            recipes: oldData.recipes.map((c: any) => {
              ...
              if (cId === recipeId) {
                const updatedPostedPlatforms = [...(c.posted_platforms || []), platform];
        ...
        const error = result.failed_platforms.find((f: any) => f.platform.toLowerCase() === platform.toLowerCase());
        ...
        const skipped = result.skipped_platforms?.find((s: any) => s.platform.toLowerCase() === platform.toLowerCase());
        if (skipped) {
          toast({
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh]">
        ...
        >
          <Loader2 className="h-12 w-12 text-blue-500/50" />
        </motion.div>
        <p className="mt-4 text-muted-foreground animate-pulse">Loading approved content...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-transparent pb-12">
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-50 via-indigo-50 to-cyan-50 mb-12 shadow-sm border border-white/20">
        <motion.div
          animate={{
            y: [0, -20, 0],
            x: [0, 10, 0],
          ...
          className="absolute top-0 right-0 -mr-20 -mt-20 w-96 h-96 bg-blue-200/30 rounded-full blur-3xl mix-blend-multiply filter pointer-events-none"
        />
        <motion.div
          animate={{
            y: [0, 20, 0],
          ...
          className="absolute bottom-0 left-0 -ml-20 -mb-20 w-96 h-96 bg-cyan-200/30 rounded-full blur-3xl mix-blend-multiply filter pointer-events-none"
        />
        ...
        <div className="relative z-10 px-8 py-16 md:py-20 text-center max-w-4xl mx-auto space-y-6">
          ...
          >
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-gray-900 mb-4 font-serif">
              Ready to <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-indigo-500 to-cyan-500 animate-gradient-x">Post</span>
            </h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Finalize and publish your approved AI-generated content to your social media channels with just one click.
            </p>
          </motion.div>
          ...
          >
            <Badge className="text-base px-5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white border-none shadow-lg shadow-blue-200/50">
              <Send className="h-4 w-4 mr-2" />
              {content.length} Ready for Publishing
            </Badge>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <div className="bg-white/60 backdrop-blur-xl rounded-2xl p-6 shadow-sm border border-slate-200/60 sticky top-4 z-20 transition-all duration-300">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="relative w-full md:w-96 group">
              ...
              <Input
                ...
                className="pl-10 h-12 bg-white/50 border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 rounded-xl transition-all shadow-sm group-hover:shadow-md group-hover:bg-white"
                value={searchQuery}
            ...
            <div className="flex flex-wrap items-center gap-2 justify-center md:justify-end">
              <Button
                ...
                className={`rounded-full px-4 h-9 font-medium transition-all duration-300 ${activePlatformFilter === 'all'
                  ? 'bg-slate-900 text-white shadow-lg shadow-slate-200 hover:bg-slate-800 scale-105'
              ...
              {Object.entries(platforms).map(([key, info]) => {
                ...
                return (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => setActivePlatformFilter(isActive ? 'all' : key)}
                    className={`rounded-full px-3 h-9 transition-all duration-300 border ${isActive
                  ...
                  >
                    <Icon className={`h-4 w-4 mr-1.5 ${isActive ? info.color : 'text-slate-400'} `} />
                    <span className={isActive ? 'font-semibold text-slate-900' : ''}>{info.name}</span>
                  </Button>
                );
              })}
            </div>
          ...
          {content.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-2xl font-bold mb-2 text-slate-800">
                {searchQuery || activePlatformFilter !== 'all' ? 'No matches found' : 'All Clear!'}
              </h3>
              ...
              {!(searchQuery || activePlatformFilter !== 'all') && (
                <Button onClick={() => window.location.href = '/cms/review'} variant="default" size="lg" className="gap-2 rounded-full shadow-lg hover:shadow-xl transition-all">
                  Go to Review
              ...
              {(searchQuery || activePlatformFilter !== 'all') && (
                <Button
                  onClick={() => { setSearchQuery(''); setActivePlatformFilter('all'); }}
                  variant="outline"
                  className="gap-2 mt-4 hover:bg-slate-100"
                >
                  Clear Filters
                </Button>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
              {content.map((item, index) => (
                ...
                >
                  <Card className="overflow-hidden border-slate-200/60 shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 h-full flex flex-col rounded-2xl hover:ring-2 hover:ring-blue-500/20 bg-white">
                    {/* Header Image Area */}
                    <div className="relative h-56 overflow-hidden bg-slate-100 group">
                      ...
                      <div className="absolute top-3 left-3 z-10">
                        <Badge className="bg-white/95 text-blue-700 backdrop-blur-md shadow-sm border-none hover:bg-white px-3 py-1">
                    ...
                    <CardContent className="flex-1 p-5 flex flex-col gap-5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {item.generated_content && Object.keys(item.generated_content).map((platform) => {
                          ...
                          const isPosted = item.posted_platforms?.includes(platform);

                          // Hide platforms not matching filter if active
                          if (activePlatformFilter !== 'all' && platform !== activePlatformFilter) return null;
                          ...
                          return (
                            ...
                            >
                              ...
                              `}>
                                <Icon className={`h-4 w-4 ${isPosted ? 'text-green-600' : (isActive ? platformInfo.color : 'text-slate-400')} `} />
                              </div>
                              {isPosted && (
                      ...
                      {/* Preview Area */}
                      <div className="flex-1 bg-slate-50 rounded-xl border border-slate-100 p-3 shadow-inner">
                        {item.generated_content && (() => {
                          const currentPlatform = selectedPlatforms[item.recipe_id] || Object.keys(item.generated_content)[0];

                          // If filter is active and current platform isn't the filtered one, select the filtered one
                          const effectivePlatform = activePlatformFilter !== 'all' && Object.keys(item.generated_content).includes(activePlatformFilter)
                            ? activePlatformFilter
                            : currentPlatform;

                          const content = item.generated_content[effectivePlatform];
                          ...
                          return (
                            <SocialMediaPreview
                              ...
                              content={displayText}
                              image={effectivePlatform === 'pinterest' && item.pinterest_image_url ? item.pinterest_image_url : item.image_url}
                      ...
                      <div className="pt-2 mt-auto">
                        {item.generated_content && (() => {
                          ...
                          const platformInfo = platforms[effectivePlatform as keyof typeof platforms];
                          const isPosted = item.posted_platforms?.includes(effectivePlatform);
                          ...
                          return (
                            <Button
                              ...
                              onClick={() => postContent(item.recipe_id, effectivePlatform)}
                            >
                              {/* Shimmer Effect */}
                              {!isPosted && !isPosting && (
                      ...
                      {activePlatformFilter === 'all' && item.generated_content && Object.keys(item.generated_content).some(p => !item.posted_platforms?.includes(p)) && (
                        <Button
                          variant="ghost"
                          size="sm"
                          className="w-full text-xs text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors"
                          onClick={() => {
                            Object.keys(item.generated_content).forEach((platform) => {
                              if (!item.posted_platforms?.includes(platform)) {
        ...
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base">
              To post to <span className="font-semibold text-slate-900">{authDialog.platform}</span>, you need to connect your account first.
              <br /><br />
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-sm">
                1. Click "Connect {authDialog.platform}" to open the login page.<br />
                2. Complete the authentication.<br />
                3. Come back here and click "I've Connected, Retry".
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="bg-muted/50 p-3 rounded-xl text-xs break-all font-mono mb-4 border border-slate-100/50 text-slate-400">
            {authDialog.authUrl}
          </div>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setAuthDialog(prev => ({ ...prev, isOpen: false }))} className="rounded-full">
              Cancel
            </AlertDialogCancel>
            <Button
              variant="outline"
